...
const command_list = 'ps -e -o pid,user,pcpu,rss,pmem,etime,args | grep -E "node" | grep -v get_node_processes | grep -v grep';
  ...
  const processList = stdio.stdout.toString().split('\n')
    .reduce((arr, line) => {
      if (!!line && !line.includes('/bin/sh') && line.includes('node')) {
        ...
        if (m) {
          const item = { pid: m[0], user: m[1], cpu: m[2], rss: m[3], pmem: m[4], time: m[5], args: m.slice(6).join(' ') };
...
exports.echoTable = (items) => {
  ...
  var action = new Table(header, body);
